...
import { estate, propertyUse } from 'data/form';
...
export const PropertyUse = () => {
  ...
  const handleTileClick = (tileText: string) => {
    setStep((currValue) => [3, { ...currValue[1], propertyUse: tileText }]);
  };

  return (
    <SimpleGrid columns={[1, 2, 3, 3]} spacing={6} justifyItems={'center'} textAlign='center'>
      {propertyUse.data.map((entry) => {
        const isDisabled = step[1].estate === estate.data[2].text && entry.text === propertyUse.data[2].text;
        ...
        return (
          <Button
            ...
            w={[300, 200, 200, 300]}
            sx={step[1].propertyUse === entry.text ? { bgColor: 'primary.acid', boxShadow: '2xl' } : {}}
            _hover={
              !isDisabled
                ? {
                    cursor: 'pointer',
          ...
          >
            <VStack
              ...
              sx={step[1].propertyUse === entry.text ? { filter: 'brightness(0) invert(1)' } : {}}
            >
